...
class InscripcionEqus extends Component
{
    ...
    {
        if ($this->currentImageName && Storage::disk('public')->exists($this->currentImageName)) {
            return Storage::url($this->currentImageName);
        }

        return null;
    }

    public function getImageUrlForInscripcion($pagoInsEquPath)
    {
        if ($pagoInsEquPath && Storage::disk('public')->exists($pagoInsEquPath)) {
            return Storage::url($pagoInsEquPath);
        }
