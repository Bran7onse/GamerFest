                ...
                <div class="card-header">
                    <div style="display: flex; justify-content: space-between; align-items: center;">
                        <div class="float-left">
                            <h4><i class="fab fa-laravel text-info"></i> INSCRIPCION INDIVIDUAL </h4>
                        </div>
                            ...
                            <tbody>
                                @foreach($inscripcionInds as $row)
                                    <tr>
                                        ...
                                        <td>
                                            @if ($row->pago_ins)
                                                <img src="{{ asset('storage/' . $row->pago_ins) }}" alt="Pago Inscripción" style="max-width: 50px; max-height: 50px;">
                                            @else
                                                Sin imagen
                                            @endif
                                        </td>
                                        <td width="90">
                                            <div class="btn-group">
                                                ...
                                                <div class="dropdown-menu dropdown-menu-right">
                                                    <a data-toggle="modal" data-target="#updateModal" class="dropdown-item" style="color:white; background-color:blue;" wire:click="edit({{$row->id}})">
                                                        <i class="fa fa-edit"></i> Editar
                                                    </a>
                                                    <a class="dropdown-item" style="color:white; background-color:red;" onclick="confirm('Confirm Delete Inscripcion Equ id {{$row->id}}? \nDeleted Inscripcion Equs cannot be recovered!')||event.stopImmediatePropagation()" wire:click="destroy({{$row->id}})">
